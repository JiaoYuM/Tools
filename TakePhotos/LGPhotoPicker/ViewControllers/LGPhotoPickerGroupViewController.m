...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        tableView.tableFooterView = [[UITableViewHeaderFooterView alloc] init];
...
- (void)setNavBack {
    [[UINavigationBar appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName:[UIColor whiteColor],NSFontAttributeName:[UIFont boldSystemFontOfSize:autoFontSize(17)]}];
    [self.navigationController.navigationBar setTintColor:[UIColor whiteColor]];
    // 设置导航栏
//    [[UIBarButtonItem appearance] setBackButtonTitlePositionAdjustment:UIOffsetMake(NSIntegerMin, NSIntegerMin) forBarMetrics:UIBarMetricsDefault];
//
    // 设置导航栏
    [[UIBarButtonItem appearance] setBackButtonTitlePositionAdjustment:UIOffsetMake(-2*WIDTH, 0) forBarMetrics:UIBarMetricsDefault];
...
- (void)addNavBarCancelButton{
    UIBarButtonItem *temporaryBarButtonItem= [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self  action:@selector(cancelBtnTouched)];
